...
function updateSVGPaths() {
    ...
    Object.keys(DOMCache.paths).forEach(lineId => {
        ...
        if ((lineData.tipo === 'calle-curva-arq2' || lineData.tipo === 'calle-curva-arq2-preview') && cacheObj.base?.length >= 3) {
            ...
            // Border strokes (pLeft, pRight, capStart, capEnd) are intentionally suppressed.
            // The fill polygon alone defines the visual road. Lot perimeters are the dominant
            // border and rendering explicit stroke lines creates triple-overlap with lot edges and
            // costura dashed lines. Set all border paths to empty.
            cacheObj.base[1].setAttribute('d', 'M -999 -999');
            cacheObj.base[2].setAttribute('d', 'M -999 -999');
            if (cacheObj.base[3]) cacheObj.base[3].setAttribute('d', 'M -999 -999');
            if (cacheObj.base[4]) cacheObj.base[4].setAttribute('d', 'M -999 -999');
...
function bindPanoramaPointerEvents() {
    ...
    function handleEnd(e) {
        ...
        if (timeDiff < 500 && moveDist < 10) {
            if (isArquitecto2Active && visor360 && !isDevModePinsActive) {
...
function setupDevModes() {
    // Intercept keyboard events on input elements to prevent Pannellum's global movement controls (WASD/arrows)
